    ...
    public CompletableFuture<String> analyzeCaseAsync(BasePredictionRequest request, String patientGroup) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                log.error("Erro ao chamar a LLM OpenAI (ChatGPT)", e);
                return "Erro na analise da OpenAI: " + e.getMessage();
            }
        });
    }
    ...
    private String callOpenAI(String prompt) {
        ...
        if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
            try {
                ...
                log.warn("Erro ao fazer parse da resposta do ChatGPT: {}", response.getBody());
                return "Parecer gerado pelo ChatGPT, mas formato de resposta inesperado.";
            }
        }

        throw new RuntimeException("Falha na chamada da API da OpenAI: " + response.getStatusCode());
    }
}
